...
const DayRadioPill = ({ term, selectedTerm, ...props }: RadioProps & { selectedTerm?: string; term: string }) => {
  return (
    <Box>
      <RadioPill
        {...props}
        checked={selectedTerm ? dayjs(selectedTerm).format('H-m') === dayjs(term).format('H-m') : false}
        radioStyles={{
          px: { base: 6, md: 20 },
        }}
      >
...
const RadioColumn = ({ rowCount, children }: { rowCount: number; children: ReactNode }) => {
  if (rowCount === 0) {
    return <NoAvailableDatesIcon />;
  }

  return (
    <VStack minH={{ base: '240px', md: '300px' }} minW={{ base: '95px', md: '200px' }}>
      {children}
    </VStack>
  );
};
...
const DayRadioGroup = ({ selectedTerm, selectedDay, setSelectedTerm, availableTerms }: IProps) => {
  ...
  const morningTerms = availableTerms.filter(term => dayjs(term).isAfter(midday) && dayjs(term).isBefore(afternoon));
  const afternoonTerms = availableTerms.filter(term => dayjs(term).isAfter(afternoon) && dayjs(term).isBefore(evening));
    ...
    <HStack w='100%' justify='space-around' align='flex-start' {...group}>
      <RadioColumn rowCount={morningTerms.length}>
        {morningTerms.map(term => (
          ...
          <DayRadioPill key={term} term={term} selectedTerm={selectedTerm} {...getRadioProps({ value: term } as any)} />
        ))}
      </RadioColumn>
