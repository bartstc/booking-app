...
export const useBookedRecordsQuery = (facilityId: string, params: IBookedRecordCollectionQueryParams) => {
  return useSuspense(bookedRecordsQueryKey(facilityId, params), () =>
    accessibilityHttpService.get<IBookedRecordCollection>(buildUrl(`facilities/${facilityId}/bookings/records`, params)),
  ).data.collection;
};
