...
import { IBookingTerm } from "../../../application";
import { employeesQueryKey } from "../../../../employees/infrastructure/query";
import { IEmployeeCollection } from "../../../../employees/application";
...
export type AvailableEmployeeOption = OptionType<string> & {
  bookingTerm: IBookingTerm;
  isAvailable: boolean;
};

const AvailableEmployeeSelectAsync = ({
  bookingTerm,
  enterpriseId,
  selectedEmployeeId,
  setEmployeeId,
}: IProps) => {
  const { data, search, nextPage, status } = useAutoComplete<
  ...
  >({
    ...
    map: ({ collection }) => {
      ...
      return collection.map((employee) => ({
        ...
        bookingTerm,
        isAvailable: bookingTerm.availableEmployeeIds.includes(
          employee.employeeId
        ),
      }));
    },
  ...
  const filteredEmployees = !bookingTerm
    ...
    : data.filter(
        ({ value }) =>
          ...
          bookingTerm.unavailableEmployees.some(
            (unavailableEmployee) => unavailableEmployee.employeeId === value
          )
      );
  ...
  return (
    <Box w="100%" maxW="350px">
      <chakra.label
        opacity={!bookingTerm ? "0.45" : "1"}
        mb={2}
        display="block"
        htmlFor="available-employee-select"
      >
        <FormattedMessage
